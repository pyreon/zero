...
describe('link class building', () => {
  ...
  it('returns base class only when not active', () => {
    expect(buildClasses('nav-link', 'active', 'exact', false, false)).toBe(
      'nav-link',
    )
  })

  it('adds active class when active', () => {
    expect(buildClasses('nav-link', 'active', 'exact', true, false)).toBe(
      'nav-link active',
    )
  })

  it('adds both active and exact classes', () => {
    expect(buildClasses('nav-link', 'active', 'exact', true, true)).toBe(
  ...
  it('returns undefined when no classes', () => {
    expect(
      buildClasses(undefined, undefined, undefined, false, false),
    ).toBeUndefined()
  })

  it('only adds exact active class when exact match', () => {
    expect(
      buildClasses(undefined, undefined, 'current-page', false, true),
    ).toBe('current-page')
  })
...
describe('link click validation', () => {
  ...
  it('navigates on plain left click', () => {
    expect(shouldNavigate(false, 0, false, false, false, false, false)).toBe(
      true,
    )
  })

  it('skips if defaultPrevented', () => {
    expect(shouldNavigate(true, 0, false, false, false, false, false)).toBe(
      false,
    )
  })

  it('skips non-left clicks (middle click, right click)', () => {
    expect(shouldNavigate(false, 1, false, false, false, false, false)).toBe(
      false,
    )
    expect(shouldNavigate(false, 2, false, false, false, false, false)).toBe(
      false,
    )
  })

  it('skips modifier keys (new tab/window behavior)', () => {
    expect(shouldNavigate(false, 0, true, false, false, false, false)).toBe(
      false,
    ) // meta
    expect(shouldNavigate(false, 0, false, true, false, false, false)).toBe(
      false,
    ) // ctrl
    expect(shouldNavigate(false, 0, false, false, true, false, false)).toBe(
      false,
    ) // shift
    expect(shouldNavigate(false, 0, false, false, false, true, false)).toBe(
      false,
    ) // alt
  })

  it('skips external links', () => {
    expect(shouldNavigate(false, 0, false, false, false, false, true)).toBe(
      false,
    )
  })
})
